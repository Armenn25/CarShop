...
/// </summary>
public sealed class CarShopExceptionHandler(
    ILogger<CarShopExceptionHandler> logger,
...
) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext ctx, Exception ex, CancellationToken ct)
    {
        ...
        {
            CarShopNotFoundException => StatusCodes.Status404NotFound,
            CarShopConflictException or CarShopBusinessRuleException => StatusCodes.Status409Conflict,
    ...
    private static ErrorDto BuildErrorDto(Exception ex, bool isDev, string traceId)
    {
        ...
        switch (ex)
        {
            case CarShopNotFoundException:
            case CarShopConflictException:
            case CarShopBusinessRuleException:
                code = "entity.error";
                message = ex.Message;
                break;
